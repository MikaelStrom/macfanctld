...
char fan1_out[PATH_MAX];
char fan2_out[PATH_MAX];
...
void find_applesmc() {
	...
	sprintf(fan1_out, "%s/fan1_output", base_path);
	sprintf(fan2_out, "%s/fan2_output", base_path);
...
void scan_sensors() {
	...
	// more than 100 sensors is an error!
	while(count < SENSORS_MAX) {
		char fname[512];
	...
	if(sensor_count > 0) {
		...
		for(i = 0; i < sensor_count; ++i) {
			...
			// find in descriptions table
			int found = 0;
			for(j = 0; j < N_DESC && ! found; ++j)
